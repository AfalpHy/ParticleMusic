...
| ReplayGain | 已接入 | 设置可选按音轨、按专辑或关闭；首选标签缺失时回退另一类标签，并结合 peak/当前用户音量限制输出余量。共享输出通过播放器 `volume-gain` 应用；USB PCM 数字音量、标准硬件音量和已实现的厂商协议都接收同一首歌的 ReplayGain。曲目切换、标签补全和模式切换会重新计算，不修改 DoP/Native DSD 码流。 |
| 硬件音量协议与安全回退 | 已接入 | 标准 UAC1/UAC2 Feature Unit 支持能力探测、GET/RANGE、SET 后 readback、多声道失败回滚；厂商协议通过 `UsbVolumeProtocol` 能力抽象选择，由精确 quirk 启用，不按厂商 VID 猜测。写入失败会恢复 DAC 满电平并回退 PCM 数字音量；读取线程在一次重启后仍失败时诚实标为 `writeOnly=true, readbackVerified=false`，不会伪装成完整双向能力。 |
...
| DSD 增益补偿 | 已接入已验证协议 | 偏好范围会随播放请求和运行中调整下发。对 `ibassoDc03Pro`，补偿按每步 `0.5 dB` 换算并只写 DAC 的 DSD 音量寄存器；PCM 寄存器、DoP 标记和 Native DSD 原始数据均不修改。其他设备只有协议能力明确声明 DSD gain 后才允许使用，否则保持码流不变并安全回退。 |
